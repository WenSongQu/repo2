...
public class ExcelUtil {
    ...
    public static <T> void simpleWrite(String name, Class<T> head, String sheetName, List<T> data, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            EasyExcel.write(response.getOutputStream(), head).registerWriteHandler(new LongestMatchColumnWidthStyleStrategy()).autoCloseStream(Boolean.FALSE).sheet(sheetName).doWrite(data);
        } catch (Exception e) {
            response.reset();
